...
@Time: 2022/07/03 09:33:40
@Description: Implements some basic sub-models used in autoencoder.

Functions list:
- [x] nonlinearity # rename to `silu`
- [x] Normalize
...
def silu(x):
    # Sigmoid Linear Unit (SiLU) function, is also known as the swish function.
    # Rename from `nonlinearity` to `silu` by js2hou
...
class AttnBlock(nn.Module):
    ...
    def execute(self, x):
        ...
        w_ = jt.bmm(q, k)     # b,hw,hw    w[b,i,j]=sum_c q[b,i,c]k[b,c,j]
        w_ = w_ * (int(c)**(-0.5))
...
class Encoder(nn.Module):
    def __init__(self, *, ch=128, out_ch=3, ch_mult=(1,2,4), num_res_blocks=2,
                 ...
                 resolution=256, z_channels=3, double_z=False, **ignore_kwargs):
        super().__init__()
        self.ch = ch
        self.temb_ch = 0
...
class Decoder(nn.Module):
    def __init__(self, *, ch=128, out_ch=3, ch_mult=(1,2,4), num_res_blocks=2,
                 attn_resolutions=[], dropout=0.0, resamp_with_conv=True, in_channels=3,
                 resolution=256, z_channels=3, give_pre_end=False, **ignorekwargs):
        super().__init__()
        self.ch = ch
        self.temb_ch = 0
        self.num_resolutions = len(ch_mult)
