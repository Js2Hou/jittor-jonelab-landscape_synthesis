...
@Time: 2022/07/05 18:43:37
@Description: 

Classes to be implemente (9):
- [x] TimestepBlock
- [x] TimestepEmbedSequential
- [x] Upsample
- [x] Downsample
- [x] SiLU
- [x] ResBlock
- [x] AttentionBlock
- [x] QKVAttentionLegacy
- [x] QKVAttention
- [x] UNetModel
...
from model_jittor.ldm.modules import (
    conv_nd,
    linear,
    avg_pool_nd,
    zero_module,
...
class ResBlock(TimestepBlock):
    ...
    def execute(self, x, emb):
        ...
        if self.use_scale_shift_norm:
            ...
            # print(type(emb_out), emb_out.shape)
            h = h + emb_out
            h = self.out_layers(h)
...
class QKVAttentionLegacy(nn.Module):
    ...
    def execute(self, qkv):
        ...
        weight = jt.linalg.einsum("bct,bcs->bts", q * scale, k * scale)  # More stable with f16 than dividing afterwards
...
class UNetModel(nn.Module):
    ...
    def __init__( # NOTE: already set the default values to match ldm_orig_256.yaml
        ...
        model_channels=128, # determine the size of UNet model
        out_channels=3, 
        num_res_blocks=2,
        attention_resolutions=(32, 16, 8), # add attention blocks in 32, 16, and 8 resolution
        dropout=0,
        channel_mult=(1, 4, 8),
        conv_resample=True,
        dims=2,
        ...
        use_checkpoint=False,
        use_fp16=False,
        num_heads=8, # number of heads of attention
        num_head_channels=-1,
